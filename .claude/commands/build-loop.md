...
- **Dependency installs are the orchestrator's job.** The builder has no Bash and the checker must never write, so package installation falls to you and is not a violation of "never write code." Install **exact-pinned** (`npm install --save-exact pkg@x.y.z`) — never a caret or range on a direct dependency, and never `npm audit fix --force`. Prefer adding a dependency before the first build cycle so the checker's audit and test runs are valid.
- **Every install must be surfaced for approval.** List each package installed, upgraded, or reverted — with its exact version and one line on why — in the final summary. The user approves dependencies retroactively; silently adding one is the failure mode this rule exists to prevent. If the only way to clear a production vulnerability is a breaking major bump, stop and raise the tradeoff instead of applying it (CLAUDE.md § Dependencies).
